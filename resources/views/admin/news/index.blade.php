                        ...
                        <form action="{{ route('admin.news.index') }}" method="get">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-6 col-md-6">
                                    ...
                                    <a href="{{ route('admin.news.index') }}" class="btn btn-danger">Nhập Lại</a>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        @can('Controllers > NewsController > create')
                            <a href="{{ route('admin.news.create') }}" class="btn btn-xs btn-primary">Tạo tin tức</a>
                        @endcan
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-responsive-lg mb-0">
                                ...
                                @forelse ($news as $item)
                                    <tr>
                                        <td> {{ $i++ }} </td>
                                        <td><img src="{{ $item->image }}" width="70" height="50"
                                                 alt="{{$item->title}}"/></td>
                                        <td> {{ $item->find_index ?? '-' }} </td>
                                        <td> {{ $item->title ?? '-' }} </td>
                                        <td> {{ $item->news_url ?? '-' }} </td>
                                        <td>
                                            @if ($item->status == 1)
                                    ...
                                    <form id="delete-form-{{ $item->id }}" method="POST"
                                          action="{{ route('admin.news.destroy', $item->id) }}" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                @empty
