		...
		<div class="col-xl-12">
			<div class="card accordion accordion-rounded-stylish accordion-bordered" id="search-sec-outer">
				...
				<div class="card-body collapse accordion__body {{ $show }}" id="rounded-search-sec" data-bs-parent="#search-sec-outer">
					<form action="{{ route('booking.admin.comment') }}" method="get">
					@csrf
						...
						<div class="row">
							...
							<input type="submit" name="search" value="Tìm kiếm" class="btn btn-primary me-2"> 
								<a href="{{ route('booking.admin.comment') }}" class="btn btn-danger">Nhập Lại</a>
								...
								@forelse ($drivers as $page)
									<tr>
										...
										<td> 
										 ...
										 <td>{{ $page->title }} 
										<td>{{ $page->content }} </td>
										<td>{{ $comment_tag[$page->comment_tag] }}</td>
										...
										<td class="text-center">
												<a href="{{ route('booking.admin.commentedit', $page->id) }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
