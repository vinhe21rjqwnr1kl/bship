		...
		<div class="col-xl-12">
			<div class="card accordion accordion-rounded-stylish accordion-bordered" id="search-sec-outer">
				...
				<div class="card-body collapse accordion__body {{ $show }}" id="rounded-search-sec" data-bs-parent="#search-sec-outer">
					<form action="{{ route('price.admin.cityservice') }}" method="get">
					@csrf
			...
			<div class="card">
				<div class="card-header">
                    ...
                    @can('Controllers > UsersController > create')
                        <a href="{{ route('price.admin.cityservicecreate') }}" class="btn btn-primary">Tạo chặn DV theo Tỉnh/Thành Phố</a>
                    @endcan
                </div>
				<div class="card-body">
						...
						<table class="table table-responsive-lg mb-0">
							<thead class="">
								<tr>
									...
									<th> <strong> Chữ tìm ở Google Map </strong> </th>
							...
							<tbody>
								...
								@forelse ($Prices as $page)
									<tr>
										<td> {{ $i++ }} </td>
										<td> {{$page->agency_name}}</td>
										<td> {{$ServicesArr[$page->service_detail_id]}}</td>
 										<td> {{$page->city}}</td>
										<td> {{$page->city_search_name}}</td>
									
										<td class="text-center ">
                                        @can('Controllers > UsersController > edit')
                                            <a href="{{ route('price.admin.cityserviceedit', $page->fee_id) }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
                                        @endcan
                                        @can('Controllers > UsersController > destroy')
                                            <a href="{{ route('price.admin.cityservicedel', $page->fee_id) }}" class="btn btn-danger shadow btn-xs sharp me-1"><i class="fa fa-trash"></i></a>
                                        @endcan
                                   	 </td>
