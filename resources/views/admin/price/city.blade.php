		...
		<div class="col-xl-12">
			<div class="card accordion accordion-rounded-stylish accordion-bordered" id="search-sec-outer">
				...
				<div class="card-body collapse accordion__body {{ $show }}" id="rounded-search-sec" data-bs-parent="#search-sec-outer">
					<form action="{{ route('price.admin.city') }}" method="get">
					@csrf
						...
						<div class="row">
							...
							<input type="submit" name="search" value="Tìm kiếm" class="btn btn-primary me-2"> 
								<a href="{{ route('price.admin.city') }}" class="btn btn-danger">Nhập Lại</a>
			...
			<div class="card">
				<div class="card-header">
                    ...
                    @can('Controllers > UsersController > create')
                        <a href="{{ route('price.admin.citycreate') }}" class="btn btn-primary">Tạo giá theo Tỉnh/Thành Phố</a>
                    @endcan
                </div>
				<div class="card-body">
								...
								@forelse ($Prices as $page)
									<tr>
                                        ...
                                        @can('Controllers > UsersController > edit')
                                            <a href="{{ route('price.admin.cityedit', $page->fee_id) }}" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
                                        @endcan
                                        @can('Controllers > UsersController > destroy')
                                            <a href="{{ route('price.admin.citydelete', $page->fee_id) }}" class="btn btn-danger shadow btn-xs sharp me-1"><i class="fa fa-trash"></i></a>
                                        @endcan
                                   	 </td>
