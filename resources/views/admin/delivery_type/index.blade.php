...
@section('content')
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        @can('Controllers > UsersController > create')
                            <a href="{{ route('delivery_type.admin.create') }}" class="btn btn-primary">Tạo loại vận
                                chuyển</a>
                        @endcan
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-responsive-lg mb-0">
                                ...
                                @forelse ($types as $type)
                                    <tr>
                                        <td> {{ $i++ }} </td>
                                        <td> {{ $type->delivery_type_name }} </td>
                                        <td>
                                            @if ($type->delivery_type_status == 1)
                                                <span class="badge badge-success"> Hoạt động</span>
                                            @else
                                                <span class="badge badge-warning"> Ngừng hoạt động</span>
                                            @endif
                                        </td>
                                        <td> {{ $type->delivery_type_ratio }} </td>
                                        @if($type->delivery_type_service_detail_id)
                                            <td> {{ $type->main_name }} -- {{ $type->type_name }} </td>
                                        @else
                                            <td> Chưa chọn dịch vụ </td>
                                        @endif
                                        <td class="text-center ">
                                            @can('Controllers > UsersController > edit')
                                                <a href="{{ route('delivery_type.admin.edit', $type->delivery_type_id) }}"
                                            ...
                                            @can('Controllers > UsersController > destroy')
                                                <button type="button" class="btn btn-danger shadow btn-xs sharp me-1"
                                                        ...
                                                        data-bs-target="#exampleModal" data-bs-id="{{$type->delivery_type_id}}">
    ...
    <script>
        ...
        exampleModal.addEventListener('show.bs.modal', function (event) {
            ...
            var deleteLinkHref = "{{ route('delivery_type.admin.destroy', ':id') }}";
