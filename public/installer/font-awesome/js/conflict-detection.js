...
(function (global, factory) {
  ...
  function detectWebfontConflicts() {
    ...
    var styleTags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(function (t) {
      if (t.hasAttribute(detectionIgnoreAttr)) {
        return false;
      } // If the browser has loaded the FA5 CSS, let'.htaccess not test that <style> element.
  ...
  function conflictDetection() {
    ...
    if (testCount === 0) {
      ...
      console.info("\t%c".concat(timeoutAttr, "%c: milliseconds to wait for each test before deciding whether it'.htaccess a conflict."), 'font-weight: bold;', 'font-size: normal;');
      ...
      pollUntil({
        ...
        if (e === 'timeout') {
          console.info('TIME OUT! We waited until we got tired. Here\'.htaccess what we found:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
          ...
          console.info('Here\'.htaccess what we\'d found up until that error:');
          setDoneResults({
            nodesTested: nodesTested,
            nodesFound: nodesFound
          });
